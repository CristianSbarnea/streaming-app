...
public class AudiobookSurpriseAlgorithm implements IAlgorithmsSurprise {
    // algorithm that gives the user surprise audiobook recommendations based on user history
    @Override
    public void generateSurprise(String userId) {
        ...
        for (int j = 0; j < database.streamList.size(); j++) {
            if (database.streamList.get(j).getStreamType().equals(3)) {
